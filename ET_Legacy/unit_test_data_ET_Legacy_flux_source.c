  ...
  for(int k=0; k<dirlength; k++)
    for(int j=0; j<dirlength; j++)
      for(int i=0; i<dirlength; i++) {
        ...
        randomize_primitives(&eos, rho[index], press[index], &dummy, // no need for eps
                             &vx[index], &vy[index], &vz[index],
                             &Bx[index], &By[index], &Bz[index]);

        randomize_primitives(&eos, rho_r[index], press_r[index], &dummy, // no need for eps
                             &vx_r[index], &vy_r[index], &vz_r[index],
                             &Bx_r[index], &By_r[index], &Bz_r[index]);

        randomize_primitives(&eos, rho_l[index], press_l[index], &dummy, // no need for eps
